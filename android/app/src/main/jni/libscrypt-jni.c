...
Java_derivekey_DeriveKey_scryptBridgeJNI( JNIEnv* env, jobject thiz,
	jbyteArray pass, jbyteArray salt, jobject N, jobject r, jobject p, jobject dkLen)
{
    ...
    if (libscrypt_scrypt(passphrase, passLen, saltBytes, saltLen, N_i, r_i, p_i, hashbuf, dkLen_i)) {
        switch (errno) {
            case EINVAL:
    ...
    END:
        ...
        if (saltBytes) (*env)->ReleaseByteArrayElements(env, salt, saltBytes, JNI_ABORT);
